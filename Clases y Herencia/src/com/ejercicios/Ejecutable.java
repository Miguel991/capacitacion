...
public class Ejecutable {
	...
	public static void main(String[] args) {
		...
		for(Persona unaPersona:lista){
			int indiceDeMasaCorporal= unaPersona.calcularImc(unaPersona.getPeso(), unaPersona.getAltura());
			if(indiceDeMasaCorporal == -1){
				System.out.println(unaPersona.getNombre() +" Esta por debajo de su peso ideal.");
			}else if(indiceDeMasaCorporal == 0){
				System.out.println(unaPersona.getNombre()+ " tiene el peso ideal para su estatura.");
			}else{System.out.println(unaPersona.getNombre()+ " tiene sobrepeso.");
			}
		}
		
/*
		int indiceDeMasaCorporalJuan = juan.calcularImc(juan.getPeso(), juan.getAltura());
